...
const getAllBookings = async () => {
  try {
    data.value = await ApiService.getAllBookings(localStorage.getItem('token'))
  } catch (error: any) {
    console.log(error)
    toast({
      description: error.response.data.error,
...
<template>
  ...
  <Table>
    ...
    <TableBody>
      <TableRow v-for="booking in data" :key="booking.id">
        <TableCell class="font-medium">
          {{ booking.id }}
        </TableCell>
        <TableCell>{{ booking.userId }}</TableCell>
        <!-- <TableCell>{{ booking.isAdmin ? 'Admin' : 'User' }}</TableCell> -->
